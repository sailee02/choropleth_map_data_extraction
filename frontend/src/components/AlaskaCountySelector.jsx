...
import { transformGeoJSONToPixelSpace, applyOverlayTransform, extractVerticesFromGeometry } from '../utils/geoJSONUtils';

const API_ROOT = import.meta.env.VITE_API_ROOT || "http://localhost:5001";


const ALASKA_COUNTIES = [
  { name: "North Slope Borough", geoid: "02185", region: "North", description: "Northern coast / Arctic — large borough" },
  { name: "Yukon-Koyukuk Census Area", geoid: "02290", region: "Interior", description: "Huge interior census area (east of Norton Sound)" },
  { name: "Lake and Peninsula Borough", geoid: "02164", region: "Southwest", description: "Southwest Alaska / Alaska Peninsula" },
  { name: "Ketchikan Gateway Borough", geoid: "02130", region: "Southeast", description: "Southeast panhandle (near Ketchikan)" }
];

export default function AlaskaCountySelector({ 
  imageUrl, 
  uploadId, 
  projection = "4326",
  alaskaSelection = null,
  onConfirm, 
  onCancel 
}) {
  ...
  const handleAlaskaClick = (e) => {
    if (!imageRef.current || !alaskaSelection || !containerRef.current) return;
    ...
    const scale = Math.min(
      availableWidth / alaskaSelection.width,
      availableHeight / alaskaSelection.height
    );
    
    
    const natX = alaskaSelection.x + ((clickX - padding) / scale);
    const natY = alaskaSelection.y + ((clickY - padding) / scale);
    
    
    const alaskaX = alaskaSelection.x;
    const alaskaY = alaskaSelection.y;
    const alaskaWidth = alaskaSelection.width;
    const alaskaHeight = alaskaSelection.height;
    
    if (natX < alaskaX || natX > alaskaX + alaskaWidth || natY < alaskaY || natY > alaskaY + alaskaHeight) {
      return; 
    }
    
    
    const currentCounty = ALASKA_COUNTIES[selectedPoints.length];
  ...
  const computeAlignment = async () => {
    ...
    try {
      ...
      if (data.overlayUrl) {
        setPreviewOverlay(`${API_ROOT}${data.overlayUrl}`);
      }
      ...
      if ((!bounds || !Array.isArray(bounds) || bounds.length !== 4) && shapefileData?.geojson?.features?.length > 0) {
  ...
  useEffect(() => {
    if (!imageRef.current || !alaskaSelection || !containerRef.current || !imageLoaded) return;

    const updateZoom = () => {
      if (imageRef.current && alaskaSelection && containerRef.current) {
        const containerRect = containerRef.current.getBoundingClientRect();
        ...
        const imgLeft = padding - (alaskaSelection.x * scale);
        const imgTop = padding - (alaskaSelection.y * scale);
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        <h2 style={{ marginTop: 0, marginBottom: '16px' }}>Select Alaska Counties</h2>

        <div style={{ marginBottom: '16px' }}>
          <p style={{ margin: '0 0 8px 0', fontSize: '14px', fontWeight: '500' }}>
            Step {selectedPoints.length + 1} of 5: Click on the {selectedPoints.length < 4 ? ALASKA_COUNTIES[selectedPoints.length].name.split(',')[0] : 'last'} area in the map below
          </p>
          <p style={{ margin: '0', fontSize: '12px', color: '#666' }}>
            Select areas matching the reference image (1: North Slope, 2: Yukon-Koyukuk, 3: Lake and Peninsula, 4: Ketchikan Gateway)
          </p>
        </div>
        ...
        <div style={{ 
              ...
              <img
                ...
                onError={(e) => {
                  ...
                  if (parent && !parent.querySelector('.fallback-text')) {
                    ...
                    fallback.innerHTML = `
                      <div style="font-weight: bold; margin-bottom: 8px;">Select in this order:</div>
                      <div>1. First anchor</div>
                      <div>2. Second anchor</div>
                      <div>3. Third anchor</div>
                      <div>4. Fourth anchor</div>
                    `;
          ...
          <div>
            <p style={{ fontSize: '12px', color: '#666', marginBottom: '8px', fontWeight: '500' }}>
              Zoomed Alaska Region: Click on counties in order
            </p>
            <div
              ref={containerRef}
              style={{
              ...
              onClick={selectedPoints.length < 4 ? handleAlaskaClick : undefined}
            >
              {!alaskaSelection ? (
                <div style={{
                  padding: '40px',
                  ...
                  Error: Alaska selection not found. Please go back and select Alaska rectangle first.
                </div>
              ) : !imageUrl ? (
                <div style={{
                  padding: '40px',
                ...
                <>
                  ...
                  {imageLoaded && selectedPoints.map((point, idx) => {
                    if (!imageRef.current || !alaskaSelection || !containerRef.current) return null;
                    ...
                    const displayX = padding + (point.x - alaskaSelection.x) * scale;
                    const displayY = padding + (point.y - alaskaSelection.y) * scale;
